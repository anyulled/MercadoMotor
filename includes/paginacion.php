...
class paginacion {
    ...
    private $pageNum;
    private $totalPages;
    public $css_clase_lista = "";
    public $css_clase_anterior = "";
    public $css_clase_siguiente = "";
    public $css_clase_pagina = "";
    ...
    public function mostrar_paginado_lista($mostrar_paginas = true) {
        ...
        $total_paginas = $this->totalPages + 1;
        // <editor-fold defaultstate="collapsed" desc="Links">
        $primero = " <a ". ($this->css_clase_anterior != "" ? " class='" . $this->css_clase_anterior . "'" : "") ." href='" . $this->currentPage . "?pageNum=0" . $this->queryString . "'>&laquo; Primero </a>";
        $anterior = " <a ". ($this->css_clase_anterior != "" ? " class='" . $this->css_clase_anterior . "'" : "") ." href='" . $this->currentPage . "?pageNum=" . max(0, $this->pageNum - 1) . $this->queryString . "'>&lsaquo; Anterior </a> ";
        $siguiente = " <a ". ($this->css_clase_siguiente != "" ? " class='" . $this->css_clase_siguiente . "'" : "") ." href='" . $this->currentPage . "?pageNum=" . min($this->totalPages, $this->pageNum + 1) . $this->queryString . "'> Siguiente &rsaquo;</a> ";
        $ultimo = " <a ". ($this->css_clase_siguiente != "" ? " class='" . $this->css_clase_siguiente . "'" : "") ."href='" . $this->currentPage . "?pageNum=" . $this->totalPages . $this->queryString . "'>&Uacute;ltimo &raquo;</a>";
        // </editor-fold>
        // <editor-fold defaultstate="collapsed" desc="lista">
        $html = "<div class='pagination'><ul". ($this->css_clase_lista != "" ? " class='" . $this->css_clase_lista . "'" : "") .">";
        if ($this->pageNum > 0) {
            $html.="<li>";
            $html.= $primero;
            $html.="</li>";
        ...
        if (!$mostrar_paginas) {
            ...
            for ($i = 1; $i <= $total_paginas; $i++) {
                $status = ($numero_pagina == $i) ? "disabled current" : "";
                $link = $this->currentPage . "?pageNum=" . ($i - 1) . $this->queryString;
                $html.="<li class='" . $status . "'><a " . ($this->css_clase_pagina != "" ? " class='" . $this->css_clase_pagina . "'" : "") . " href='$link'>" . $i . " </a></li>";
            }
        }
        if ($this->pageNum < $this->totalPages) {
            $html.="<li>";
