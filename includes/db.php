...
Class db {
    ...
    public function __construct() {
        try {
            ...
            die($exc->getMessage(). ". Consulte al administrador del sistema.");
        }
    }
...
Class Misc {
    ...
    public static function error_handler($num, $err, $file, $line) {
        ...
        $html.="<hr/>";
        $html.="<pre><h4>Entorno</h4><code>" . Misc::dump($entorno) . "</code></pre>";
        if (isset($_GET) && sizeof($_GET) > 0) {
            $html.="<hr/>";
            $html.= "<pre><h4>GET</h4><code>" . misc::dump($_GET) . "</code></pre>";
        }
        if (isset($_POST) && sizeof($_POST) > 0) {
            $html.="<hr/>";
            $html.= "<pre><h4>POST</h4><code>" . misc::dump($_POST, 1) . "</code></pre>";
        }
        if (isset($_FILES) && sizeof($_FILES) > 0) {
            $html.="<hr/>";
            $html.= "<pre><h4>Archivos cargados</h4><code>" . misc::dump($_FILES, 1) . "</code></pre>";
        }
        if (isset($_SESSION) && sizeof($_SESSION) > 0) {
            $html.="<hr/>";
            $html.= "<pre><h4>Sesion</h4><code>" . misc::dump($_SESSION, 1) . "</code></pre>";
        }

        $html.="<hr/>";
        $html.="<h1>Error.$err</h1>";
//        $html.= "<pre><b>Server:</b></pre><code>" . misc::dump($_SERVER) . "</code>";
        $html.="</fieldset>";
        // </editor-fold>
        // <editor-fold defaultstate="collapsed" desc="comprobacion de errores">
        if ((!stristr($file, "simplepie.inc") && !stristr($file, "krumo") && !stristr($err, "mysqli")) && ($num == 1 || $num == 2 || $num == 4 || $num == 8 || $num == 16 || $num == 256 || $num == 2048 || $num == 2049)) {
//            echo "<code style='border:1px solid black; background-color:white!important; border-radius:5px;'>" . $html . "</code>";
            ...
            if (MOSTRAR_ERROR)
                echo $html;
        } elseif (!stristr($file, "simplepie.inc") && !stristr($file, "krumo.php") && !stristr($err, "mysqli")) {
//            echo "<code style='border:1px solid black; background-color:white!important; border-radius:5px;'>" . $html . "</code>";
            if (EMAIL_ERROR)
                mail(EMAIL_CONTACTO, EMAIL_TITULO, $html, $headers);
            if (MOSTRAR_ERROR)
            ...
            //mail("anyulled@gmail.com", "mercadomotor.com.ve: Error", $html, $headers);
        }
        // </editor-fold>
    }
    ...
    public static function dump($args) {
        return nl2br(str_replace(
                        array(' ', ';', '[', ']'), array(' ', '; ', '<b>', '</b>'), print_r($args, true)));
    }
    ...
    public static function noticias_rss($item_limit = 4) {
        include_once SERVER_ROOT . 'includes/simplepie.inc';
        include_once SERVER_ROOT . 'includes/idn/idna_convert.class.php';
        $feed = new SimplePie();
        $feed->set_feed_url(array(
            "http://www.autonews.com.ve/feed",
            "http://www.delvolante.com/feed/",
            "http://www.f1latam.com/rss/rss.php",
            "http://feeds.feedburner.com/BlogCochescom",
            "http://feeds.feedburner.com/diariomotor"));
        $feed->item_limit = $item_limit;
        ...
        if ($feed_success) {
            foreach ($feed->get_items() as $item) {
                array_push($noticias, array(
                    "fuente" => $item->get_feed()->get_title(),
    ...
    public static function url_sortable($campo = "id", $direccion = "") {
        $params = explode("&", $_SERVER['QUERY_STRING']);
        $newParams = array();
        if (count($params) > 0) {
            foreach ($params as $param) {
                /* si no encuentro el campo ni la direccion en la url */
                if (stristr($param, $campo) === false && stristr($param, $direccion) === false) {
                    array_push($newParams, $param);
                } else {
                    $direccion = ($direccion == "") ? "desc" : $direccion;
                }
            }
    ...
    public static function mover_imagen($temp_imagen, $nombre_imagen) {
        $imagen = new SimpleImage();
        $exito = move_uploaded_file($temp_imagen, '../images/misc/' . $nombre_imagen);
        if ($exito) {
            $filename = '../images/misc/' . $nombre_imagen;
            $watermark = "../images/logo.gif";
            $destino = $filename;
            chmod($filename, 0744);
            $imagen->load($filename);
            if ($imagen->getWidth() > $imagen->getHeight()) {
                $imagen->resizeToWidth(533);
            } else {
                $imagen->resizeToHeight(400);
            }
            $imagen->save($filename);
            $imagen->merge($filename, $watermark, $destino);
        } else {
            trigger_error("No se pudo mover la imagen. ");
        }
        return $exito;
    }
